...
describe('geolocate', () => {
  testPropType(MapContainer, 'geolocate', 'bool');

  /* Why can I not get the Geosuggest here?
  it('provides the geolocate input if true', async () => {
    const wrapper = shallow(<MapContainer geolocate />);
    await wrapper.instance().componentDidMount();
    console.log(wrapper.find(MapComponent).dive().find(Map).dive().find(Geosuggest));
    expect(wrapper.find(MapComponent).dive().find(Map).dive().find(Geosuggest)).toHaveLength(1);
  });


  it('does not provide the geolocate input if false', async () => {
    const wrapper = mount(<MapContainer />);
    await wrapper.instance().render();
    const w = wrapper
      .find(MapComponent).at(0)
      .find('div').at(0)
      .find(Map).at(0)
    await w.render();
    expect(w.find(Geosuggest)).toHaveLength(0);
  });
  */
});
describe('height', () => {
